...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def computeQValueFromValues(self, state, action):
        ...
        return sum([self.mdp.getReward(state, action, pair[0]) + self.discount*pair[1]*self.getValue(pair[0]) for pair in states_probs])
    ...
    def runValueIteration(self):
        ...
        for state in self.mdp.getStates():
            for action in self.mdp.getPossibleActions(state):
                states_probs = self.mdp.getTransitionStatesAndProbs(state, action)
                for pair in states_probs:
                    if pair[1] > 0:
                        predecessors[pair[0]].add(state)

        ueue = util.PriorityQueue()
        ...
        for state in self.mdp.getStates():
            ...
            diff = abs(self.getValue(state) - max([self.getQValue(state, action) for action in actions]))

            ueue.push(state, -diff)
